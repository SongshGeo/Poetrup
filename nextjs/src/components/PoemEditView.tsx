...
interface PoemEditViewProps {
  poem: Poem;
  words: Word[];
  folders: Folder[];
  onBack: () => void;
  onUpdate: (poem: Poem) => void;
}

export function PoemEditView({ poem, words, folders, onBack, onUpdate }: PoemEditViewProps) {
  const [poemTitle, setPoemTitle] = useState(poem.title);
  const [poemDescription, setPoemDescription] = useState(poem.description || "");
  const [placedWords, setPlacedWords] = useState<PlacedWord[]>(
    // 如果有保存的位置信息，使用它；否则使用默认布局
    poem.placedWords && poem.placedWords.length > 0
      ? poem.placedWords
      : poem.wordIds.map((wordId, index) => ({
          wordId,
          x: 100 + (index % 3) * 150,
